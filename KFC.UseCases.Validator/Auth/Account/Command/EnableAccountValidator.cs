...
public class EnableAccountValidator: IInputPortValidator<EnableAccountDto>
{
	private readonly IAccountRepository _accountRepository;

	public EnableAccountValidator(IAccountRepository accountRepository)
	{
    ...
    public async Task<bool> IsValid(EnableAccountDto inputDto)
    {
        try
        {
            ...
            if (accountDisable.Status != StatusEnum.Disabled)
            {
                HttpStatusCode = HttpStatusCode.BadRequest;
                Messages = new List<MessageDto> { new MessageDto("No se puede activar un usuario que no este inactivo") };
                return false;
            }
